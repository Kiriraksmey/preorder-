...
@Service
public class AliPaymentServiceImplCDBR implements IPaymentService<PaymentRequestCyberCDBR, AlipayPaymentResponse> {
    @Value("${url.service}")
    String URL_SERVICE;

    @Value("${service.payment.alipay.cdbr}")
    String ALIPAY_PAYMENT_URI;

    @Override
    public AlipayPaymentResponse initPayment(PaymentRequestCyberCDBR paymentRequest) {
